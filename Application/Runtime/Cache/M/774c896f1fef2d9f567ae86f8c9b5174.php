...
<body>
    <header data-blend-widget="header" class="blend-header header_title_bg">
        ...
        <span class="blend-header-title">
            <a class="blend-header-item">选择检测机构</a>
        </span>
 ...
 <!--最新检测结果列表-->
    <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="blend-card" data-name='<?php echo ($vo["name"]); ?>' data-id='<?php echo ($vo["id"]); ?>'>
            <div class="blend-card-img-view">
                <img src="<?php echo ($site_url); echo ($vo["logo"]); ?>">
            </div>
            <div class="blend-card-content-view">
                <div class="blend-card-title">
                    <?php echo ($vo["name"]); ?>
                </div>
                <p class="blend-card-text"><?php echo ($vo["content"]); ?></p>
...
//        alert("fdfd");
        ...
        sessionStorage.institutionname = name;
        sessionStorage.institutionid = iid;

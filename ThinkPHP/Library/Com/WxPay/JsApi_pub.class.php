...
//	include_once("./WxPayConf_pub.class.php");
...
class Common_util_pub
{
	 ...
	 * 	作用：生成签名
	 */
	public function getSign_wxpay($Obj)
	{
		foreach ($Obj as $k => $v)
		{
			$Parameters[$k] = $v;
...
//                $String = $String."&key=lanbingqun665399015889515411lbq1";
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = WxPayConf_pub::APPID_OPEN;//公众账号ID
...
{	
	...
	public function getParameters($prepay_id)
	{
                ...
                $jsApiObj["appid"] = WxPayConf_pub::APPID_OPEN;
...
//                print_r($jsApiObj);
                $jsApiObj["sign"] = $this->getSign_wxpay($jsApiObj);
                
                
//                $this->parameters = json_encode($jsApiObj);
		$jsApiObj2["appId"] = WxPayConf_pub::APPID_OPEN;
		$jsApiObj2["partnerId"] = WxPayConf_pub::MCHID;
                $jsApiObj2["prepayId"] = $prepay_id;
                
                $jsApiObj2["nonceStr"] = $jsApiObj["noncestr"];
//		$timeStamp = time();
                $jsApiObj2["timeStamp"] = "$timeStamp";
                $jsApiObj2["package"] = "Sign=WXPay";
                $jsApiObj2["sign"] = $jsApiObj["sign"];
		return $jsApiObj2;
	}
...
* APPAPI支付——调起支付接口
*/
class App_Api_pub extends Common_util_pub
{
	var $code;//code码，用以获取openid
	var $openid;//用户的openid
	var $parameters;//jsapi参数，格式为json
